...
Use this Skill for CodexAgentTeam members. Do not use ordinary Codex task or thread messaging tools to contact them.
...
The context identifies the current member, teammates, and `team.sharedDirectory`. Write durable shared documents there with normal file tools. When a teammate needs one, send its absolute path, why it matters, and the expected action instead of copying the document into the message.
